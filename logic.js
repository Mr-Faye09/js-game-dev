...
function slideLeft(){
	for(let r=0; r<rows; r++){
		...
		for(let c=0; c<columns; c++){
            ...
            // line for animation
            // if current tile != to the originaltile, app;y for animation
...
function slideUp(){
	for(let c=0; c<columns; c++){
		...
		for(let r=0; r<rows; r++){
            ...
            // if statement to apply animetion
...
// event listener to capture touch in hte screen and assigns the x and y coordinates in the startX and startY variable.
document.addEventListener("touchstart", (e) =>{
	startX = e.touches[0].clientX;
	startY = e.touches[0].clientY;
})

// event listener to check whereyou touch the screen and prevents scrolling. input targets any element hat includes the word title
...
// listent for the touchedn event on the entire document
document.addEventListener("touchend", (e) =>{

	// check if the element trggered has a class name "tile"
	if(!e.target.className.includes("tile")){
		return; //exit the function
	}

	// calculate the difference between the inital postion nad the final t
